...
const checkPromoAccess = db.prepare(
  'SELECT id FROM promo_redemptions WHERE user_id = ? AND game_id = ? LIMIT 1'
);
...
// 게임 접근 권한 확인 (구매 또는 프로모 코드)
function hasPurchased(userId, gameId) {
  return !!checkPurchase.get(userId, gameId) || !!checkPromoAccess.get(userId, gameId);
}

// 게임 접근 유형 반환: 'purchased' | 'promo' | null
function getAccessType(userId, gameId) {
  if (checkPurchase.get(userId, gameId)) return 'purchased';
  if (checkPromoAccess.get(userId, gameId)) return 'promo';
  return null;
}
