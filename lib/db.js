...
const ready = initSqlJs().then((SQL) => {
  ...
  // Migrations — add columns missing from older DB versions
  try { rawDb.exec('ALTER TABLE users ADD COLUMN last_provider TEXT'); } catch (_) {}
  try { rawDb.exec('ALTER TABLE purchases ADD COLUMN payment_method TEXT'); } catch (_) {}
